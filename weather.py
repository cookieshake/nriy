...
class IyWeather:
    def getWeather(self, zoneCode="1141058500"):
        ...
        for dong in open("./donglist.txt", encoding="utf8").read().split("\n"):
            dong=dong.split(",")
            if dong[0]==zoneCode:
                location=" ".join(dong[1:4])
        #location=xmlraw.getElementsByTagName("category")[0].firstChild.data
    ...
    def getFive(self, areaq="서울"):
        areaq=urllib.parse.quote_plus(areaq)
        ...
        except IndexError: return "장소 오류"
        ...
        lineList=[]
        lineList.append(location+"의 향후 3일간 날씨\n")
        ...
        for forecast in forecastList[2:]:
            ...
            line="%s(%s):\n%s/▲%s℃▽%s℃/%s%%\n" % (date[5:], day, skytext, high, low, precip)
            lineList.append(line)
